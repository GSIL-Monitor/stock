...
TableStore.prototype.mutations = {
  ...
  toggleAllSelection: debounce(10, function(states) {
    ...
    readOriginal(states, data){
      this.table.$emit('readOriginal',data);
    },
    sort(states,order){
        if(states.label == order){
            if(states.sortOrder == 'desc'){
                states.sortOrder = 'asc';
            }else{
                states.sortOrder = 'desc';
            }
        }else{
            states.label = order;
            states.sortOrder = 'desc';
        }
        this.table.$emit('sort',states.sortOrder,order);
    },
    checkAll(states){
        if(states.selection.length == states.data.length){
            states.selection = [];
        }else{
            states.selection = [];
            states.data.forEach((row)=>{
        ...
        this.table.$emit('checkAll',states.selection);
    },
    toggleSingle(states,row){
        let index = states.selection.indexOf(row);
        if(index>=0){
            states.selection.splice(index,1)
        }else{
            states.selection.push(row);
        }
        this.table.$emit('toggleSingle',states.selection);
    },
    clickTitle(states,row,index){
        this.table.$emit('clickTitle',row,index);
    }
};
