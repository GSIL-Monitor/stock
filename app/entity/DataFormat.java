	...
	public static void setVal(Map<String, Object> map, String hKey, Map<String, Object> result, String pattern,String dKey,List<String> apiNameList) {
		...
		if(result == null || result.isEmpty() || result.get(dKey) == null || result.get(dKey).toString().isEmpty()){//格式化空值
			...
			if(time != null){//格式化日期格式
				...
				//朝阳智投好公司列表report_date字段特殊处理
				if (apiNameList.contains("v1/zyzt/good_company_list") && "report_date".equals(dKey)) {
					int quarter = DateUtil.getQuarter(time);
		...
		for(String api : apiNameList){
			switch (api) {
				...
				case "v1/select_stock/get_stock_tags": 
					if ("over_rate_so".equals(key)) {// over_rate_so：当季超额收益率
						value = getFormatDoubleValue(value, "0.01/0.00%");
					}
					break;
				case "v1/zyzt/good_company_list"://朝阳智投, 本期好公司
					String fieldsName = "turnover_rate,today_yield,week_yield,month_yield,quarter_yield,year_yield";
					if ("report_date".equals(key)) {
//						DateUtil.getQuarter();
					} else if ("price_change_rate".equals(key)) {
						value = getFormatDoubleValue(value, "0.01/0.00%");
					} else if ("pe_y1".equals(key) || "turnover".equals(key) || "price".equals(key)) {
						value = getFormatDoubleValue(value, "0.00");
					} else if ("tcap".equals(key)) {
						value = getFormatDoubleValue(value, "0.0001/0.00");
					} else if (fieldsName.contains(key)) {
						value = getFormatDoubleValue(value, "1.0/0.00%");
					}
					break;
				default:
	...
	public static Object getFormatDoubleValue(Object value, String format){
		try {
			if(value instanceof Double || value.toString().matches("^(-?\\d+)(\\.\\d+)?(E-?\\d+)?$")){
				Double valueTmp;
